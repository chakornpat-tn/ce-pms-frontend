...
export function ProgressReportDetailDialog({
  children,
  progressId,
  projectId,
  onSuccess,
  ...
  const IsAdvisor = useSWR(`check-advisor-${projectId}`, () =>
    CheckUserIsAdvisor(projectId),
  )
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] w-[95vw] max-w-4xl overflow-y-auto rounded bg-white p-4 md:h-auto md:p-6">
        <DialogHeader>
          ...
          <DialogDescription>
            แสดงรายละเอียดและผลการประเมินรายงานความคืบหน้าของโครงการ
          </DialogDescription>
        </DialogHeader>
        <div className="mx-auto max-w-2xl space-y-4 md:space-y-6">
          <div className="space-y-2">
            {data?.reportResult && <ReportResultDisplay data={data} />}
          </div>
          <div className="space-y-3 md:space-y-4">
            ...
            <div className="space-y-2">
              {!data?.reportResult && IsAdvisor.data && (
                <>
...
const EvaluationForm: React.FC<{ id: number; onSuccess?: () => void }> = ({
  id,
  onSuccess,
}) => {
  const [error, action, isPending] = useActionState(
    (prevState: unknown, formData: FormData) => {
      UpdateProgressReport(prevState, formData)
      onSuccess?.()
    },
    null,
  )
    ...
    <div className="mx-auto max-w-4xl rounded-md bg-white p-6 shadow-md">
      ...
      <form action={action}>
        <input
          type="hidden"
          name="status"
          value={progressReportStatus.REPLIED}
        />
        ...
        <div className="mb-4">
          {[
            ...
            <div key={index} className="mt-3">
              ...
              <div className="mt-2 flex gap-4">
                {['น้อย', 'ปานกลาง', 'มาก'].map((option, idx) => (
          ...
          <div className="mb-4">
            ...
            <ul className="list-inside list-disc">
              <li>น้อย เทียบเท่ากับ 3 คะแนน</li>
              <li>ปานกลาง เทียบเท่ากับ 4 คะแนน</li>
              <li>มาก เทียบเท่ากับ 5 คะแนน</li>
            </ul>
          </div>
          <div className="mt-3">
        ...
        <button
          type="submit"
          disabled={isPending}
          className={`w-full rounded-md py-2 text-white shadow-md ${
            isPending
              ? 'cursor-not-allowed bg-gray-400'
              : 'bg-primary2-400 hover:bg-primary2-500'
          }`}
        >
...
const ReportResultDisplay = ({ data }: { data: ProgressReport }) => {
  return (
    <div className="mx-auto max-w-4xl rounded-md bg-white p-6 shadow-md">
      <h1 className="mb-4 text-xl font-bold">
        ผลการประเมินรายงานความก้าวหน้า โครงการวิศวกรรม/วิจัย
