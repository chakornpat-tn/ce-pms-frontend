  ...
  return (
    <div className="flex w-full flex-col gap-2">
      {userRole === userRoles.Teacher ? (
        <Link href={`/teacher/consultant/${projectId}/docs`}>
          <button className={commonButtonClasses}>
            <AssignmentIcon className="shrink-0" /> <span className="flex-1 text-center">ตรวจสอบเอกสาร</span>
          </button>
        </Link>
      ) : (
        <button className={commonButtonClasses}>
      ...
      {courseMenu === course.Project && (
        <button className={commonButtonClasses}>
          <ProgressIcon className="shrink-0" /> <span className="flex-1 text-center">ตรวจสอบความคืบหน้า</span>
        </button>
      )}
      ...
      {userRole === userRoles.Teacher ? (
        <div className="space-y-2">
          ...
          >
            <button className={commonButtonClasses}>
              <CheckCircleIcon className="shrink-0" /> <span className="flex-1 text-center">อนุมัติสอบ</span>
            </button>
          </UpdateCourseStatusDialog>
          <button className={commonButtonClasses}>
            <GradeIcon className="shrink-0" /> <span className="flex-1 text-center">ส่งคะแนนสอบ</span>
          </button>
        </div>
      ) : (
        <button className={commonButtonClasses}>
          <PreviewIcon className="shrink-0" /> <span className="flex-1 text-center">ตรวจคะแนนสอบ</span>
        </button>
      )}
    </div>
  )
